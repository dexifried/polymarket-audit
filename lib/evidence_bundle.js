...
function normalizeContextSnippets(input) {
  ...
  return input
    .map((item) => {
      if (typeof item === 'string') return sanitizePromptInput(item.trim());
      if (item && typeof item.text === 'string') return sanitizePromptInput(item.text.trim());
      return '';
    })
    .filter(Boolean)
    .slice(0, 8);
...
export function buildEvidenceBundle(candidate, state = {}, opts = {}) {
  ...
  return {
    market: {
      ...
      question: sanitizePromptInput(market?.question || null),
      endDate: market?.end_date_iso || market?.endDate || null,
      description: sanitizePromptInput(market?.description || market?.rules || null),
      tags: arr(market?.tags).map((tag) => sanitizePromptInput(typeof tag === 'string' ? tag : tag?.label || tag?.name)).filter(Boolean),
      category,
    ...
    memory: sanitizePromptInput(opts.memoryContext || null),  // Enriched by memory_retrieval.getContextForPosition
  };
}
  ...
  try {
    ...
    return {
      ...
      memory: {
        ...
        topSimilarFacts: (ctx.similarTrades || []).slice(0, 3).map(r => sanitizePromptInput(r.text?.slice(0, 120))),
        foresightCount: ctx.relevantForesights?.length || 0,
        topForesights: (ctx.relevantForesights || []).slice(0, 3).map(r => sanitizePromptInput(r.text?.slice(0, 120))),
